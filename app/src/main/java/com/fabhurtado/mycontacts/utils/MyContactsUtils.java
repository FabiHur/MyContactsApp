...
public class MyContactsUtils {
    ...
    private File createVcfFile() {

        String name = mContact.getFirstName() + " " + mContact.getLastName();

        File vcf = new File(getActivity().getExternalFilesDir(null), name + ".vcf");
        try {
            FileWriter writer = new FileWriter(vcf);
            writer.write("BEGIN:VCARD\r\n");
            writer.write("VERSION:3.0\r\n");
            writer.write("N:" + mContact.getLastName() + ";" + mContact.getFirstName() + "\r\n");
            writer.write("FN:" + name + "\r\n");
            writer.write("TITLE:" + mContact.getBirthDate() + "\r\n");
            writer.write("PHOTO;VALUE=URI;TYPE=PNG:" + mContact.getThumb());

            for (ContactPhone cp : mContact.getPhones()) {
                if(cp.getNumber() != null){
                    if (cp.getType().equals("Home")){
                        writer.write("TEL;TYPE:HOME,VOICE:" + cp.getNumber() + "\r\n");
                    }
                    if (cp.getType().equals("Cellphone")){
                     writer.write("TEL;TYPE:CELL,VOICE:" + cp.getNumber() + "\r\n");
                     }
                    if (cp.getType().equals("Office")){
                        writer.write("TEL;TYPE:WORK,VOICE:" + cp.getNumber() + "\r\n");
                    }
                }
            }

            for (ContactAddress ca : mContact.getAddresses()){
                if(ca.getHome() != null){
                    writer.write("ADR;TYPE=HOME,PREF:;;" + ca.getHome() + "\r\n");
                }
                if (ca.getWork() != null){
                    writer.write("ADR;TYPE=WORK:;;" + ca.getWork() + "\r\n");
                }
            }

            writer.write("REV:" + mContact.getCreatedAt() + "\r\n");
